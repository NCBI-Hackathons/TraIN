...
###            c. if count in immunoGeneCounts.csv>=30 then recode as 1, otherwise 0.
###            d. calcualte expression percentage which using sum(# of 1)/9 for each cell type.
###            e. if percentage is >=0.375 then noted as 1 (expressed), otherwise 0.
...
# if larger than 30 then assign to 1
cell_b_numeric_1 <- ifelse(cell_b_numeric>=30, 1, 0)
...
cell_b_numeric_4 <- cell_b_numeric_3[,2:ncol(cell_b_numeric_3)]/8

# cut off percentage 37.5% 
cell_b_numeric_5 <- ifelse(cell_b_numeric_4>=0.375, 1, 0)

# combine cell type
cell_b_numeric_6 <- cbind(cell_b_numeric_3[,1], cell_b_numeric_5 )

# output result 
#write.csv(cell_b_numeric_6, "Expression_pct_by_cell_type_for_each_gene_30_reads_cutoff_and_37.5_pct_cutoff.csv", row.names=F)
